...
// FineTunedCompletionRequest contains all relevant fields for requests to the completions endpoint,
// using a fine-tuned model.
//
// Note: This seems completely redundant, and may change (or be removed) if a better / simpler solution arrises.
type FineTunedCompletionRequest struct {
	// Model specifies the ID of the model to use.
	// See more here: https://beta.openai.com/docs/models/overview
	Model models.FineTunedModel `json:"model"`
	// Prompt specifies the prompt(s) to generate completions for, encoded as a string, array of strings, array of
	// tokens, or array of token arrays. Note that <|endoftext|> is the document separator that the model sees during
	// training, so if a prompt is not specified the model will generate as if from the beginning of a new document.
	// Defaults to <|endoftext|>.
...
// FineTunedCompletionResponse is the response from the completions endpoint.
//
// Note: This seems completely redundant, and may change (or be removed) if a better / simpler solution arrises.
type FineTunedCompletionResponse struct {
	ID      string                `json:"id"`
	Object  objects.Object        `json:"object"`
	Created uint64                `json:"created"`
	Model   models.FineTunedModel `json:"model"`
...
func (c *Client) CreateFineTunedCompletion(ctx context.Context, cr *FineTunedCompletionRequest) (*FineTunedCompletionResponse, error) {
	var b, err = c.post(ctx, routes.Completions, cr)
	if err != nil {
		return nil, err
	}

	var resp = &FineTunedCompletionResponse{}
	if err = json.Unmarshal(b, resp); err != nil {
		return nil, err
	}
